...
//Get the customer ID from the primary/home phone number they entered into the form
$result = dbconnect_newmethod()->query('SELECT cust_id FROM customer WHERE cust_hphone LIKE "'.$_POST['cust_hphone'].'"');
...
//Query to get the just-inserted svc_address record
$result = dbconnect_newmethod()->query('SELECT sa_id 
																					FROM svc_address 
																				 WHERE cust_id='.$cust_id.'
																				   AND sa_lastupd = (SELECT MAX(sa_lastupd)
																					 										 FROM svc_address
																															WHERE cust_id='.$cust_id.')
																			 ');
$row = $result->fetch_assoc();
