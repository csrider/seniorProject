<?php 
	...
	$result = dbconnect_newmethod()->query("SELECT * FROM customer_type WHERE ctype_id='".$_POST['ctype_id_toedit']."'");
	...
	$_SESSION['ctype_id'] = $_POST['ctype_id_toedit'];
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td valign="top" align="center">
			...
			<!-- Edit follows -->
			<form action="ctype_edit.php" method="post">
			<table cellpadding="2" cellspacing="0" border="0" width="100%">
				<tr bgcolor="#aaaaaa">
					<td colspan="2"><h4>Edit an Existing Customer Type</h4></td>
				</tr>
				...
				<tr><td><br /></td></tr>
				<tr>
					<td align="right">Change Type Name:</td>
					<td align="left">
						<input 
							type="text" 
							name="ctype_type" 
							maxlength="32" 
							size="33" 
							value="<?php echo $row1['ctype_type']; ?>" 
						/>
					</td>
				</tr>
				<tr>
					<td align="right">Change Type Description:</td>
					<td align="left">
						<input 
							type="text" 
							name="ctype_desc" 
							maxlength="128" 
							size="65" 
							value="<?php echo $row1['ctype_desc']; ?>" 
						/>
					</td>
				</tr>
				<tr>
...
function showctype_edit() {
	$result = dbconnect_newmethod()->query("SELECT * FROM customer_type WHERE ctype_id='".$_POST['ctype_id_toedit']."'");
	$num_results = $result->num_rows;
	for ($i=0; $i<$num_results; $i++) {
		$row2 = $result->fetch_assoc();
		echo '<td align="left">'.$row2['ctype_type'].'</td>';
		echo '<td>'.$row2['ctype_desc'].'</td>';
		echo '<td>'.$row2['ctype_lastupd'].'</td>';
		echo '<td>'.$row2['ctype_created'].'</td>';
	}
}
?>
