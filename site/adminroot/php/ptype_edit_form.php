<?php 
	...
	$result = dbconnect_newmethod()->query("SELECT * FROM prod_type WHERE ptype_id='".$_POST['ptype_id_toedit']."'");
	...
	$_SESSION['ptype_id'] = $_POST['ptype_id_toedit'];
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td valign="top" align="center">
			...
			<!-- Edit follows -->
			<form action="ptype_edit.php" method="post">
			<table cellpadding="2" cellspacing="0" border="0" width="100%">
				<tr bgcolor="#aaaaaa">
					<td colspan="2"><h4>Edit an Existing Product Type</h4></td>
				</tr>
				...
				<tr>
					...
					<td align="left">
						<input 
							...
							maxlength="32" 
							size="33" 
							value="<?php echo $row1['ptype_type']; ?>" 
						/>
					</td>
				</tr>
				<tr>
					...
					<td align="left">
						<input 
							...
							maxlength="128" 
							size="65" 
							value="<?php echo $row1['ptype_desc']; ?>" 
						/>
					</td>
				</tr>
				<tr>
...
function showptype_edit() {
	$result = dbconnect_newmethod()->query("SELECT * FROM prod_type WHERE ptype_id='".$_POST['ptype_id_toedit']."'");
	$num_results = $result->num_rows;
	for ($i=0; $i<$num_results; $i++) {
		$row2 = $result->fetch_assoc();
		echo '<td align="left">'.$row2['ptype_type'].'</td>';
		echo '<td>'.$row2['ptype_desc'].'</td>';
		echo '<td>'.$row2['ptype_lastupd'].'</td>';
		echo '<td>'.$row2['ptype_created'].'</td>';
	}
}
?>
